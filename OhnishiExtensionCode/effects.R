  ...
  for (j in 1:J) {
    for (i in 1:N[J]) {
      ...
      if (C[ij]==1) {
        ...
        Sig0 <- calcSigma(2*sum(N), phi[,R[ij]], rbind(W0, Whl[[R[ij]]]))
        theta0_mean <- Sig0[1:sum(N), (sum(N)+1):(sum(N)*2)]%*%
          chol2inv(chol(Sig0[(sum(N)+1):(sum(N)*2), (sum(N)+1):(sum(N)*2)]))%*%
          theta_true[,R[ij]]
        theta0_var <- Sig0[1:sum(N), 1:sum(N)] - 
          (Sig0[1:sum(N), (sum(N)+1):(sum(N)*2)]%*%
          solve(Sig0[(sum(N)+1):(sum(N)*2), (sum(N)+1):(sum(N)*2)], tol=1e-100)%*%
          Sig0[(sum(N)+1):(sum(N)*2), 1:sum(N)])
