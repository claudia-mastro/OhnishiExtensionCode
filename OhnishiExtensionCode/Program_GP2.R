...
for(s in 2:mcmc_samples){
  ...
  print("BETA")
   for(k in 1:3){
      ...
      cov_beta<-tryCatch(
        expr = {
          chol2inv(chol(crossprod(W_k)/sigma2[(s-1), k] + diag(5)/sigma2_beta))
        },
        error = function(e){
          chol2inv(chol(nearPD(crossprod(W_k)/sigma2[(s-1), k] + diag(5)/sigma2_beta, 
   ...
   for(k in 4:6){
     ...
     omega_inv <- tryCatch(
       expr = {
       chol2inv(chol(omega[[s-1,k-3]]))
       },
       error = function(e){
         chol2inv(chol(nearPD(omega[[s-1,k-3]], base.matrix = TRUE)$mat))
       }
     )
     Sigma_inv <- tryCatch(
       expr = {       
         chol2inv(chol(Sigma[[s-1,k-3]]))
        },
       error = function(e){
        chol2inv(chol(nearPD(Sigma[[s-1,k-3]], base.matrix = TRUE)$mat))
       }
     )
     ...
     cov_mu <- tryCatch(
       expr = {
          chol2inv(chol(cov_mu))
       },
       error = function(e){
         chol2inv(chol(nearPD(cov_mu, base.matrix = TRUE)$mat))
       }
     )
     mean_mu<-cov_mu%*%mu_sum
     mu456[[k-3]][s,] <- rmnorm(1, mean=mean_mu, varcov=cov_mu)
     beta_cov <- chol2inv(chol(kronecker(diag(sum(N)), crossprod(W)) + 
                                 kronecker(Sigma_inv, omega_inv)))
     beta_mu <- beta_cov %*% (c(t(rep(1, sum(N))%*%crossprod(Y_long, W)))/sigma2[(s-1), k] +
                chol(chol2inv(kronecker(Sigma_inv, omega_inv)))%*%c(t(rep(1, sum(N))%*%t(mu456[[k-3]][s,]))))
     beta_mu <- matrix(beta_mu, nrow=sum(N), byrow=T)
     beta[[k]][[s]] <- matrix(rmnorm(1, mean=beta_mu, varcov=beta_cov), nrow=sum(N))

     rate<-crossprod(Y_k - diag(W_k%*%t(beta[[k]][[s]])))/2.00 +
       b_sigma2
     shape<-length(Y_k)/2.00 +
       a_sigma2
     sigma2[s,k]<-1.00/rgamma(n = 1,
     ...
     omega[[s, k-3]] <- tryCatch(
       expr = {
         rinvwishart(sum(N)+6, omega_scale)
       },
       error = function(e){
         rinvwishart(sum(N)+6, nearPD(omega_scale, base.matrix=TRUE)$mat)
       }
     )
   ...
   for(k in 4:6){
     ...
     omega_inv <- tryCatch(
       expr = {
         chol2inv(chol(omega[[s,k-3]]))
       },
       error = function(e){
         chol2inv(chol(nearPD(omega[[s,k-3]], base.matrix = TRUE)$mat))
       }
     )
     Sigma_inv <- tryCatch(
       expr = {       
         chol2inv(chol(Sigma[[s,k-3]]))
       },
       error = function(e){
         chol2inv(chol(nearPD(Sigma[[s,k-3]], base.matrix = TRUE)$mat))
       }
     )
     ...
     Sigma_new_inv <- tryCatch(
       expr = {
          chol2inv(chol(Sigma_new[[s,k-3]],tol=1e-22))
       },
       error = function(e){
         chol2inv(chol(nearPD(Sigma_new[[s,k-3]],base.matrix=TRUE)$mat))
       }
     )
     cov_mu<- diag(5)/(sigma2_beta)
     mu_sum<-0
     ...
     omega_new_inv <- tryCatch(
       expr = {
          chol2inv(chol(omega_new))
       },
       error = function(e){
         chol2inv(chol(nearPD(omega_new, base.matrix = TRUE)$mat))
       }
     )
...
#save.image(file="/home/cim24/project/OhnishiExtension/JWCode/GP_full_new.RData")
