...
if (file.exists(paste0("~/palmer_scratch/OhnishiExtension/Results/", v, "/CADE", i, ".rds"))) {
  ...
  CADE <- readRDS(paste0("~/palmer_scratch/OhnishiExtension/Results/", v, "/CADE", id, ".rds"))
  CASE <- readRDS(paste0("~/palmer_scratch/OhnishiExtension/Results/", v, "/CASE", id, ".rds"))
  
  CADE.lower[i] <- quantile(CADE, 0.025, na.rm=TRUE)
  CADE.upper[i] <- quantile(CADE, 0.975, na.rm=TRUE)
  CASE.lower[i] <- quantile(CASE, 0.025, na.rm=TRUE)
  CASE.upper[i] <- quantile(CASE, 0.975, na.rm=TRUE)
  CADE.coverage[i] <- (eff[[1]] >= quantile(CADE, 0.025, na.rm=TRUE)) & (eff[[1]] <= quantile(CADE, 0.975, na.rm=TRUE))
  CASE.coverage[i] <- (eff[[2]] >= quantile(CASE, 0.025, na.rm=TRUE)) & (eff[[2]] <= quantile(CASE, 0.975, na.rm=TRUE))
  
  CADE.abs.bias[i] <- median(CADE-eff[[1]])
  CASE.abs.bias[i] <- median(CASE-eff[[2]])
  CADE.bias[i] <- median(CADE-eff[[1]])/eff[[1]] * 100
  CASE.bias[i] <- median(CASE-eff[[2]])/eff[[2]] * 100
  CADE.var[i] <- var(CADE)
  CASE.var[i] <- var(CASE)
}
}
