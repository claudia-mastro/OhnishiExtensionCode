...
W_true<-list(cbind(x_long, S_long/N_long, a_long, Z_long),
        cbind(x_long, S_long/N_long, a_long, Z_long),
        cbind(x_long, S_long/N_long, a_long, Z_long),
        cbind(x_long, S_long/N_long, a_long, Z_long, h4_true),
        cbind(x_long, S_long/N_long, a_long, Z_long, l5_true),
        cbind(x_long, S_long/N_long, a_long, Z_long, h6_true, l6_true))


psi2_true <- runif(1)
phi_true <- list(runif(4), runif(4), runif(4), runif(5), runif(5), runif(6))
...
theta_true <- cbind(rmnorm(1, -4, varcov=Sigma_true[[1]]*psi2_true),
                   rmnorm(1, -2, varcov=Sigma_true[[2]]*psi2_true),
                   rmnorm(1, -1, varcov=Sigma_true[[3]]*psi2_true),
                   rmnorm(1, 0, varcov=Sigma_true[[4]]*psi2_true),
                   rmnorm(1, 2, varcov=Sigma_true[[5]]*psi2_true),
                   rmnorm(1, 4, varcov=Sigma_true[[6]]*psi2_true))
...
for(j in 1:sum(N)) {
  if (a_long[j] == eff.a & S_long[j] == eff.s & R_a_long_true[j] == 3) {
    ...
    if (Z_long[j] == 0) {
      ...
      theta1 <- rmnorm(1, mean = 0, varcov = psi2_true*Sig1)
      Y1_long[j] <- rnorm(n = 1,
      ...
      theta0p <- rmnorm(1, mean = 0, varcov = psi2_true*Sig0p)
      ...
      theta0 <- rmnorm(1, mean = 0, varcov = psi2_true*Sig0)
      Y0_long[j] <- rnorm(n = 1,
